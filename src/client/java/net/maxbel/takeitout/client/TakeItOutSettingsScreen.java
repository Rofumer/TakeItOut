...
public class TakeItOutSettingsScreen extends Screen {
    ...
    private void renderAllItems(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.text(this.font, "Sources: " + WorldContainerSources.size(), listLeft + 6, listTop - 12, 0xFFA7F3D0);

        if (items.isEmpty()) {
            guiGraphics.centeredText(
                    this.font,
                    WorldContainerSources.size() == 0 ? "No linked containers" : "No items found",
                    this.width / 2,
                    listTop + 36,
                    0xFFAAAAAA
            );
    ...
    private void renderContainers(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        if (hoveredSource != null) {
            renderContainerContentsTooltip(guiGraphics, hoveredSource.pos(), mouseX, mouseY);
        }
    }
    ...
    private void renderContainerContentsTooltip(GuiGraphicsExtractor guiGraphics, BlockPos source, int mouseX, int mouseY) {
        ...
        for (WorldContainerSources.SourceEntry source : sources) {
            ...
            if (y >= listTop && y + 22 <= listBottom && mouseY >= y + 2 && mouseY < y + 20) {
                if (mouseX >= linkButtonX && mouseX < linkButtonX + 58) {
                    if (WorldContainerSources.setLinked(this.minecraft, source.pos(), !source.linked())) {
                        requestItems();
                    }
                    return true;
                }
                ...
                if (mouseX >= deleteButtonX && mouseX < deleteButtonX + 54) {
                    if (WorldContainerSources.delete(this.minecraft, source.pos())) {
                        requestItems();
                    }
                    return true;
                }
    ...
    private void requestItems() {
        ...
        net.fabricmc.fabric.api.client.networking.v1.ClientPlayNetworking.send(
                new Takeitout.GetWorldContainerItemsPayload(sourcePositions)
        );
    }
    ...
    private List<Takeitout.WorldContainerItemCount> getSortedItems() {
        ...
        items.sort(Comparator.comparing(item -> item.stack().getHoverName().getString()));
        return items;
    }
    ...
    private ItemStack getContainerIcon(BlockPos source) {
        ...
        return this.minecraft.level.getBlockState(source).getBlock().asItem().getDefaultInstance();
    }
